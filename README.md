...
本版集中修复第三方代码审查中确认的问题，重点是**配置生效链路**与**转录落盘性能**。

- **修复设置面板改配置后不生效**：LLM 的模型 / 密钥 / 超时此前只在服务构造时读取一次，
  改完必须重启后端，而界面却提示「已同步到当前进程」，容易把人引向错误方向。
  现改为按配置签名自动重建客户端，**保存即生效**。
- **修复转录落盘的全量重写**：此前每收到一句新转录就重写整个文件，
  2 小时课程约 1500 行时写入量接近行数的平方，且发生在 ASR 回调的锁内。
  现改为**增量追加**，仅在会话标记 / 滚动摘要变化时才整体重写。
- **修复最近转录可能混入上一节课内容**：转录行时间戳不含日期，
  旧记录只要落在「最近 N 分钟」窗口内就会被误取。现只取**最后一次会话**的内容。
- **修复上传老格式文件没有有效提示**：`.ppt` / `.doc` 此前列在白名单里但必然解析失败。
  现明确提示「请先另存为 .pptx / .docx」；上传临时文件也改用唯一文件名，失败时一并清理。
- **修复后端版本号长期脱节**：`/docs` 页显示的版本停在 `1.0.1`。
  现统一取自 `config.APP_VERSION`。
- **清理与去重**：删除 Tauri 模板残留的 `greet` 命令、两个无引用的死样式、
  `requirements.txt` 里的重复依赖与未使用依赖。
...
本版为**问题修复版**，内容来自第三方代码审查（逐条核实后处理）。按类别分组：

- **配置生效**：LLM 客户端改为按签名自动重建 —— 改模型 / 密钥 / 超时不再需要重启后端；
  设置保存的提示文案补充「`API_PORT` 变更仍需重启」。
- **转录链路**：落盘由全量重写改为增量追加（消除 O(n²) IO）；
  最近转录只取最后一次会话，避免上一节课的残留行混入救场 / 进度总结上下文。
- **资料上传**：`.ppt` / `.doc` 从白名单移除并给出「另存为 .pptx / .docx」引导；
  临时文件改用 `mkstemp` 唯一命名，成功与失败路径都清理。
- **版本号来源**：后端新增 `config.APP_VERSION`，`/docs` 页不再显示陈旧的 `1.0.1`。
- **清理**：删除 Tauri 模板残留的 `greet` 命令、`.startup-glow` / `.startup-dot`
  两个无引用死样式及配套 keyframes、`requirements.txt` 重复依赖与未使用的 `aiofiles`。
- **版本号**：2.0.4 → 2.0.5。
